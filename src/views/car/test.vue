  ...
  <div class="app-container">
    <el-form label-width="75px">
      <el-form-item label="组织名称">
        ...
        <el-form-item label="接受器ID">
          <el-input v-model="ruleForm.receiverId" placeholder="请输入接受器ID"></el-input>
        </el-form-item>
		<el-form-item label="监控状态">
		  <el-select  v-model="ruleForm.monitorStatus" clearable>
			<!-- <el-option label="全部" value="" />
		    <el-option label="正常" :value="1000" />
			<el-option label="低压报警" :value="1001" />
			<el-option label="高温报警" :value="1003" />
			<el-option label="高压报警" :value="1002" />
			<el-option label="电池电压低报警" :value="2001"/>
			<el-option label="无信号报警" :value="3001" />
			<el-option label="二级低压报警" :value="4001" />
			<el-option label="二级高温报警" :value="4003" />
			<el-option label="二级高压报警" :value="4002" /> -->
			<el-option
    ...
    <el-radio-group v-model="ruleForm.versionType" @change="getList">
      ...
      <el-radio-button :label="5001">硬件版</el-radio-button>
	  <el-radio-button :label="5002">纯软件版</el-radio-button>
    </el-radio-group>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain  :disabled="multiple" @click="handleAdd"
          v-hasPermi="['system:dict:add']">车辆调拨</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain  @click="handleUpdate()">车辆增加</el-button>
      </el-col>
	  <el-col :span="1.5">
	    <el-button
	       type="info"
    ...
    <el-table  :data="tableData" v-loading="loading"  @selection-change="handleSelectionChange">
      ...
      <el-table-column label="接收器ID"  align="center" style="cursor: pointer;" width="180">
      ...
      <el-table-column label="车型" align="center" prop="carClassName" />
      <el-table-column label="GPS累计里程(km)" align="center" prop="totalMileage" />
      <el-table-column label="软件版累计里程(km)" align="center" prop="remark" />
      ...
      <el-table-column label="创建时间" align="center" prop="createdTime" />
        <el-table-column label="操作" align="center" class-name="small-padding" width="150">
		   <template #default="scope">
			  ...
			  @click="handleDelete(scope.row.vehicleId)"><Delete style="width: 1em; height: 1em; margin-right: 2px"></Delete>删除</el-button>
			</template>
        </el-table-column>
    </el-table>
   <pagination
   ...
   />
    <!-- 轮胎详情 -->
    <el-dialog :title="title" v-model="open" width="80%" append-to-body :close-on-click-modal="false">
        <el-form label-width="130px" :model="form" :rules="rules" ref="menuRef" :validate-on-rule-change="false">
          <div class="formdiv">
			...
			<el-form-item label="累计里程" prop="totalMileage">
			  <el-input placeholder="单位km" v-model="form.totalMileage"></el-input>
			</el-form-item>
          </div>
          <div class="formdiv">
			...
			<el-form-item label="拖车车型" v-if="form.truckType  != 10" prop="trailCarType">
			  <el-select v-model="trailCarType" clearable @change="trailCarTypechange">
			 <el-option
           ...
           <el-form-item label="初始公里表"  prop="initMileage">
              <el-input  v-model="form.initMileage" placeholder="单位km"></el-input>
            </el-form-item>
          </div>
	...
	<el-dialog :title="upload.title" v-model="upload.open" width="400px" append-to-body :close-on-click-modal="false">
	   ...
	   <el-radio-group v-model="versionType">
	     <el-radio :label="5001">硬件版</el-radio>
	     <el-radio :label="5002">纯软件版</el-radio>
	   </el-radio-group>
	   <template #footer>
...
<script setup lang="ts">
	...
	import {carlist,addcar, carinfo, updatecar, detalcar, gofeedi, exportcarlist, binds} from '@/api/car.js'
	import {carcompanylist,vehicleNo} from '@/api/systensettings'
	import {actionslist} from '@/api/company.js'
	import { getToken } from "@/utils/auth";
	const { proxy } = getCurrentInstance();
	const { car_class, gaucar_class, alarm_type } = proxy.useDict("car_class", "gaucar_class","alarm_type");
	...
	function trailCarTypechange(val){
		let [id,name] = val.split(',')
			form.value.trailCarType =Number(id) 
			form.value.trailCarTypeName = name
	}
	...
	const handleUpdate=function(row){
			 if(row){
				 ...
				 carinfo(row.vehicleId).then(res=>{
					...
					trailCarType.value=res.data.trailCarType + "," + res.data.trailCarType//未处理的gua车型
...
// 解码下载方法
	...
	const handleFileSuccess = (response, file, fileList) => {
	  ...
	  proxy.$alert("<div style='overflow: auto;overflow-x: hidden;max-height: 70vh;padding: 10px 20px 0;'>导入成功" + response.data + "</div>", "导入结果", { dangerouslyUseHTMLString: true });
	  getList();
	};
	/** 提交上传文件 */
