    ...
    <div class="app-container">
        <div class="top">
            <el-radio-group v-model="report" @change="changeReport">
                <el-radio-button label="detail">轮胎月度成本明细表</el-radio-button>
                <el-radio-button label="summary">轮胎月度成本汇总表</el-radio-button>
            </el-radio-group>
        </div>
        ...
        <div v-show="report == 'summary'">
            <el-form label-width="75px" class="indexforn">
                <el-form-item label="月度">
                    <el-date-picker format="YYYY/MM" value-format="YYYY-MM" v-model="queryformDetail.startTime" type="month"
                        placeholder="请选择月度" />
                </el-form-item>
                <el-form-item label="轮胎分类">
...
// 统计方式改变
// function mehidchange(e){
// 	console.log(e)
// 	// console.log(queryformDetail.value.modeTypes.includes(1))
// }
//处理方法
...
function exportable() {
    ...
    expor(queryform.value).then(res => {
        exportxlxs(res.data).then(respon => {
            ...
            downloadFiles(respon,'轮胎月度明细表.xlsx')
        })
    })
}
// 导出2
function exportabletwo(){
	...
	contlistexport(queryformDetail.value).then(res=>{
		exportxlxs(res.data).then(respon => {
		    downloadFiles(respon,'轮胎月度汇总表.xlsx')
		})
	})
}
// 解码下载方法
