    ...
    <div class="app-container">
        <div class="top">
            <el-radio-group v-model="report" @change="changeReport">
                <el-radio-button label="detail">轮胎年度成本明细表</el-radio-button>
                <el-radio-button label="summary">轮胎年度成本汇总表</el-radio-button>
            </el-radio-group>
        </div>
        ...
        <div v-show="report == 'summary'">
            <el-form label-width="75px" class="indexforn">
                ...
                <el-form-item label="统计方式">
                    <el-select v-model="modeTypes" multiple placeholder="请选择统计方式" style="width: 240px">
...
<script setup name="TireYearCost" lang="ts">
...
function exportable() {
   ...
   expor(queryform.value).then(res => {
        exportxlxs(res.data).then(respon => {
            ...
            downloadFiles(respon,'轮胎年度明细表.xlsx')
        })
    })
}
// 导出2
function exportabletwo(){
	...
	contlistexport(queryformDetail.value).then(res=>{
		exportxlxs(res.data).then(respon => {
		    downloadFiles(respon,'轮胎年度汇总表.xlsx')
		})
	})
}
// 解码下载方法
