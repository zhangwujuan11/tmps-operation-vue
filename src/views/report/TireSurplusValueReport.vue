<template>
  <div class="app-container">
   ...
   <el-table :data="dataList">
	 ...
	 <el-table-column label="初测花纹深度(mm)" align="center" prop="firstDepth" />
	 <el-table-column label="实测花纹深度(mm)" align="center" prop="measuredDepth" />
	 <el-table-column label="单价" align="center" prop="price" />
     <el-table-column label="每毫米磨损(元/mm)" align="center" prop="wearPerMillimeter"/>
     <el-table-column label="剩余价值(元)" align="center" prop="surplusValue" />
   </el-table>
   <pagination
      v-show="total > 0"
      :total="total"
   ...
   <p class="titles">
	 <span>
		 ...
		 平均单价：{{totalinfo.avgPrice}}元    &emsp;平均每毫米磨损：{{totalinfo.avgWearPerMillimeter}}(元/mm)  &emsp;剩余平均价值：{{totalinfo.avgSurplusPrice}}元
	  </span>
   </p>
   <p class="titles">
   	 <span>
   		  <img src="@/assets/r_icon1.png">
   		 合计总数：{{total}}条    &emsp;合计总金额：{{totalinfo.allPrice}}元    &emsp;合计总剩余价值：{{totalinfo.allPrice}}元
   	  </span>
   </p>
...
<script setup lang="ts" name="Tiremileageestimatelist">
	...
	import { assetDetailslist, assetexport } from '@/api/reop'
	...
	const { lifecycle_tire_category, lifecycle_tire_sendertype } = proxy.useDict("lifecycle_tire_category", "lifecycle_tire_sendertype");
	const queryform=ref({
		pageNum:1,
		pageSize:10,
		vehicleNo:'',
	...
	// 获取table数据
	function getList(){
		assetDetailslist(queryform.value).then(res=>{
	...
	function exportable(){
		...
		assetexport(queryform.value).then(res=>{
	...
	function downloadFiles(data) {
	    if (typeof window.chrome !== 'undefined') {
	        ...
	        link.download = "轮胎动态资产明细.xlsx";
	        link.click();
	    } else if (typeof window.navigator.msSaveBlob !== 'undefined') {
	        // IE version
	        var blob = new Blob([data], { type: 'application/force-download' });
	        window.navigator.msSaveBlob(blob, "轮胎动态资产明细.xlsx");
	    } else {
	        // Firefox version
	        var file = new File([data], "轮胎动态资产明细.xlsx", { type: 'application/force-download' });
