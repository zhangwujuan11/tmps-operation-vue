...
<script setup lang="ts" name="Tirepatternabnormallist">
	...
	function downloadFiles(data) {
	    if (typeof window.chrome !== 'undefined') {
	        ...
	        link.download = "花纹搭配异常列表.xlsx";
	        link.click();
	    } else if (typeof window.navigator.msSaveBlob !== 'undefined') {
	        // IE version
	        var blob = new Blob([data], { type: 'application/force-download' });
	        window.navigator.msSaveBlob(blob, "花纹搭配异常列表.xlsx");
	    } else {
	        // Firefox version
	        var file = new File([data], "花纹搭配异常列表.xlsx", { type: 'application/force-download' });
